...
var GATE_AGENT_TIMEOUT_MS = 5 * 60 * 1e3;
...
async function runGateAgent(config, bundle, _parentSignal) {
...
var ExitPlanModeToolInvocation = class extends BaseToolInvocation {
  ...
  async getDefaultPermission() {
    ...
    if (isAutonomousPrePlanMode(prePlanMode) && gateState && gateState.enteredByModel && gateState.gateMode !== "user_takeover") {
      return "allow";
    }
    return "ask";
  }
  ...
  buildRejectedGateDisplay(message, plan, details) {
    return {
      type: "plan_summary",
      message,
      plan: `${plan.trimEnd()}
...
${notes}` : "")
  ...
  fallbackToUserDecision(plan) {
    ...
    return {
      llmContent: "Gate is unavailable and cannot review the plan. Ask the user whether to execute this plan or stay in plan mode to revise it.",
      returnDisplay: {
        type: "plan_summary",
        message: "Plan gate is unavailable. The plan has been saved \u2014 please confirm whether to execute it.",
        plan
      }
    };
  }
